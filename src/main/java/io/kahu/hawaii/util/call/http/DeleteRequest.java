...
public class DeleteRequest<T> extends AbortableHttpRequest<T> {

    public DeleteRequest(RequestDispatcher requestDispatcher, RequestContext<T> context, URI uri, ResponseHandler<HttpResponse, T> responseHandler,
            CallLogger<T> logger) {
        super(requestDispatcher, context, responseHandler, new HttpDelete(uri), logger);
    }

    public DeleteRequest(RequestPrototype prototype, URI uri) {
        super(prototype, new HttpDelete(uri));
    }
}
