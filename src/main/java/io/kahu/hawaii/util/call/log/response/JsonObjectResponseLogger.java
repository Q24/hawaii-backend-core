...
public class JsonObjectResponseLogger implements ResponseLogger<JSONObject> {
    @Override
    public void logResponse(LogManager logManager, Response<JSONObject> response) {
        String type = response.getRequest().getCallName();
        String id = response.getRequest().getId();
        String body = null;
        JSONObject o = response.getResponsePayload();
        if (o != null) {
            try {
