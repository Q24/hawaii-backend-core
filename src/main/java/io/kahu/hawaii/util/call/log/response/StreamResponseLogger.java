...
public class StreamResponseLogger implements ResponseLogger<OutputStream> {
    @Override
    public void logResponse(LogManager logManager, Response<OutputStream> response) {
        String type = response.getRequest().getCallName();
        String id = response.getRequest().getId();
        String body = null;
        if (response.getResponsePayload() != null) {
            body = "Response outputstream of type: " + response.getResponsePayload().getClass().getName();
        }
